...
function change(v, province, city, district) {
    ...
    with (ss) {
        ...
        if (v && document.getElementById(s[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for(var d=0;d<ar.length;d++){
                    if(ar[d].slice(ar[d].length - 1) == '镇'){
                        ar.splice(d,1);
                    }
                }
        ...
        if (++v < s.length) {
            change(v, province, city, district);
        }
    }//End with
}
...
var s = null;//三个select的name
//var opt0 = ["省份","地级市","市、县级市"];//初始值
var opt0 = ["省份", "地级市", "市、县级市"];//初始值
function _init_area(province, city, district, ss) {  //初始化函数
    s = ss;
